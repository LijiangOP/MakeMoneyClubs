...
public class DailyFragment extends MVPBaseFragment<DailyContract.View, DailyPresenter> implements DailyContract.View, NewHandMissionAdapter.CallBack {
    ...
    protected View getFragmentView() {
        View view = View.inflate(getActivity(), R.layout.daily_fragment, null);
    ...
    public void showView(NewBieHttpEntity httpResult) {
        try {
            if (httpResult.code == Constant.HttpResult.SUCCEED) {
                ...
                if (mAdapter == null) {
                    mAdapter = new NewHandMissionAdapter(getActivity(), mData, this);
                ...
                ToastUtil.showToast(getActivity(), TextUtils.isEmpty(httpResult.msg) ? "获取数据失败" : httpResult.msg);
    ...
    public void onGetGameList(GameListHttpResult httpResult) {
        ...
        if (httpResult.code == Constant.HttpResult.SUCCEED) {
            ...
            if (games != null && games.size() > 0) {
                mGameListDialog = new GameListDialog(getActivity());
                mGameListDialog.initView(games, new View.OnClickListener() {
                    ...
                    public void onItemClick(int type) {
                        SkipUtils.SkipGame(type, getActivity());
                        mGameListDialog.dismiss();
                    }
                });
