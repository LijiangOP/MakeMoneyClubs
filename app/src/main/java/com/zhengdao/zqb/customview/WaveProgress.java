...
import com.zhengdao.zqb.utils.MiscUtil;
...
public class WaveProgress extends View {
    ...
    private static final String TAG                 = "littlejie";
    ...
    private void init(Context context, AttributeSet attrs) {
        mDefaultSize = DensityUtil.dip2px(context, DEFAULT_SIZE);
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mDarkWaveAnimTime = typedArray.getInt(R.styleable.WaveProgress_darkWaveAnimTime, DEFAULT_ANIM_TIME);
        mLightWaveAnimTime = typedArray.getInt(R.styleable.WaveProgress_lightWaveAnimTime, DEFAULT_ANIM_TIME);
        mMaxValue = typedArray.getFloat(R.styleable.WaveProgress_maxValue, DEFAULT_MAX_VALUE);
        mValue = typedArray.getFloat(R.styleable.WaveProgress_value, DEFAULT_VALUE);
        mValueSize = typedArray.getDimension(R.styleable.WaveProgress_valueSize, DEFAULT_VALUE_SIZE);
        ...
        mHintSize = typedArray.getDimension(R.styleable.WaveProgress_hintSize, DEFAULT_HINT_SIZE);

        mCircleWidth = typedArray.getDimension(R.styleable.WaveProgress_circleWidth, DEFAULT_ARC_WIDTH);
        ...
        mWaveHeight = typedArray.getDimension(R.styleable.WaveProgress_waveHeight, DEFAULT_WAVE_HEIGHT);
        mInsideMargin = typedArray.getDimension(R.styleable.WaveProgress_inside_circle_margin, 0);
        mTextMargin = typedArray.getDimension(R.styleable.WaveProgress_text_margin, 0);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int minSize = Math.min(getMeasuredWidth() - getPaddingLeft() - getPaddingRight() - 2 * (int) mCircleWidth, getMeasuredHeight() - getPaddingTop() - getPaddingBottom() - 2 * (int) mCircleWidth);
        mRadius = minSize / 2;
    ...
    private void drawCircle(Canvas canvas) {
        ...
        int currentAngle = (int) (360 * (mCurrentValue / mMaxValue));
        //画背景圆环
    ...
    private void drawWave(Canvas canvas, Paint paint, Point[] points, float waveOffset) {
        ...
        mWavePath.close();
        mWaveLimitPath.addCircle(mCenterPoint.x, mCenterPoint.y, mRadius - mInsideMargin, Path.Direction.CW);
    ...
    private void drawProgress(Canvas canvas) {
        float y = mCenterPoint.y - (mPercentPaint.descent() + mPercentPaint.ascent()) / 2 - mTextMargin;//高度
        if (mPrePercent == 0.0f || Math.abs(mPercent - mPrePercent) >= 0.01f) {
            mPercentValue = String.format("%.0f%%", mPercent * 100);
            mPrePercent = mPercent;
        }
        //百分比进度文字
        canvas.drawText(String.format("%.0f%%", mCurrentValue), mCenterPoint.x, y, mPercentPaint);
    ...
    private void startAnimator(final float start, float end, long animTime) {
        Log.i(TAG, "startAnimator,value = " + mValue + ";start = " + start + ";end = " + end + ";time = " + animTime);
